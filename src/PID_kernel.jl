function main_loop_original_kernel_withPID(
            ...
            if iteration <= 10 || mod(iteration - 1, termination_evaluation_frequency) == 0 || (iteration >= iteration_limit)
                ...
                if idx==1
                    if restart_choice == RESTART_CHOICE_NO_RESTART
                        ...
                        if last_restart_primal_distance_moved > 1e-16 && last_restart_dual_distance_moved > 1e-16 && last_restart_primal_distance_moved< 1e12 && last_restart_dual_distance_moved < 1e12 && ratio_infeas > 1e-8 && ratio_infeas < 1e8
                            restart_error = log(last_restart_dual_distance_moved) - log(last_restart_primal_distance_moved) - log(primal_weight[1])
