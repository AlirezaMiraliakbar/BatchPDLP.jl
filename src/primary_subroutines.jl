...
function PDLP(
    ...
    version::Symbol=:rHalpern,
    return_dual_obj::Bool=false,
    return_both_obj::Bool=false,
    global_upper_bound::Float64=Inf
)
    if version
        not in [:rHalpern, :original]
        error("Version not recognized. Must be :rHalpern or :original")
    end
    ...
    elseif PDLP_data.parameters.scale_initial_primal_weight
        select_initial_primal_weight(PDLP_data.primal_weight, PDLP_data.original_problem, PDLP_data.dims, version)
    end
    ...
    # Call the main PDLP kernel based on the set version 
    if version == :rHalpern
        CUDA.@sync @cuda blocks = PDLP_data.dims.n_LPs threads = max_req shmem = max_size * sizeof(Float64) main_loop_rHalpern_kernel(
            solutions,
            objectives,
    ...
    elseif version == :original
        CUDA.@sync @cuda blocks = PDLP_data.dims.n_LPs threads = max_req shmem = max_size * sizeof(Float64) main_loop_original_kernel(
            solutions,
            objectives,
            PDLP_data.original_problem.variable_lower_bounds,
            PDLP_data.original_problem.variable_upper_bounds,
